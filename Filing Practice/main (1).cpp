...
void filing_create (const Saving_Acc&, char*);
		...
		else if (login_option == 2)
		{
			...
			else if (account_option == 2)
			{
				...
				if (retry == 1)
				{
					...
					outfile<<temp.getName()<<endl;
					outfile<<temp.getAcc_Num()<<endl;
...
void filing_create (const Saving_Acc &s1, char *filename)
{
    ...
    std::ofstream write2(filenames.c_str(), std::ios::out | std::ios::trunc);
